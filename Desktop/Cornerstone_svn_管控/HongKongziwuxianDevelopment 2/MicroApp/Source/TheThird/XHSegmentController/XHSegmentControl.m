...
- (void)setSelectIndex:(NSInteger)selectIndex animation:(BOOL)animation
{
    ...
    if (selectIndex < self.items.count) {
        ...
        if ([self.delegate respondsToSelector:@selector(forMyXhSegmentSelectAtIndex:animation:)]) {
            [self.delegate forMyXhSegmentSelectAtIndex:selectIndex animation:animation];
        }
