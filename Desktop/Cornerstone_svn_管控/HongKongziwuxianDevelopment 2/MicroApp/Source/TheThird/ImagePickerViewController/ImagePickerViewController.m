...
@interface ImagePickerViewController () <UIGestureRecognizerDelegate>
{
    CGFloat _viewImgFramWidth;  //  图片缩放原尺寸
...
@property (nonatomic, assign)CGRect circularFrame;//裁剪框的frame   圆形
...
-(void)CreatUI
{
    ...
    if (_clipType == originalImage) {
        ...
        self.circularFrame = CGRectMake(0, 0, KScreenWidth,KScreenHeight - KScreenNavigationBarHeight - KScreenTabBarIndicatorHeight);
    } else {
...
-(void)drawClipPath:(ClipType )clipType
{
    ...
    {
        [path appendPath:[UIBezierPath bezierPathWithRect:CGRectMake(center.x - self.radius, center.y - ((self.radius)*9/16), self.radius * 2,(self.radius * 2)*9/16)]];
        
        self.circularFrame = CGRectMake(center.x - self.radius, center.y - ((self.radius)*9/16), self.radius * 2,(self.radius * 2)*9/16);
    }
