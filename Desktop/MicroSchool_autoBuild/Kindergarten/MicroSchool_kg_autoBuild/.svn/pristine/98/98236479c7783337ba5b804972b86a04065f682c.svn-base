...
+ (NSMutableArray*)getGifFrames:(NSData*)gifImageData {
	...
	//[[gifFrames retain] autorelease];//update
	//[gifImageView release];//update
	return gifFrames;
}
...
- (void)setGIF_frames:(NSMutableArray *)gifFrames {
	//[gifFrames retain];//update
	
	if (GIF_frames) {
		//[GIF_frames release];//update
	}
...
- (void)loadImageData {
	...
	//[overlayArray release];//update
	//[array release];//update
...
- (void) decodeGIF:(NSData *)GIFData {
    ...
    if (GIF_buffer != nil)
    {
       // [GIF_buffer release];//update
    }
    
    if (GIF_global != nil)
    {
       // [GIF_global release];//update
    }
    
    if (GIF_screen != nil)
    {
       // [GIF_screen release];//update
    }
...
- (bool) GIFGetBytes:(int)length {
    if (GIF_buffer != nil)
    {
        //[GIF_buffer release]; // Release old buffer//update
        GIF_buffer = nil;
    }
...
//		GIF_buffer = (NSMutableData *)[[GIF_pointer subdataWithRange:NSMakeRange(dataPointer, length)] retain];
        GIF_buffer = (NSMutableData *)[GIF_pointer subdataWithRange:NSMakeRange(dataPointer, length)];//update
        dataPointer += length;
		return YES;
	}
    else
