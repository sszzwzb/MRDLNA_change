...
//业务库实例
static MomentsListDBDao *gDaoInstance = nil;

@implementation MomentsListDBDao
...
+ (MomentsListDBDao *)getDaoInstance
{
    ...
    {
        if (gDaoInstance == nil) {
            gDaoInstance = [[MomentsListDBDao alloc] init];
            //初始化数据库名字
...
{
    //创建上报表
    NSString *reportSql = [DBTableCreate getMomentsListSql];
...
-(NSDictionary*)getData:(NSString*)type page:(NSString*)page{
    ...
    NSString *sql = [NSString stringWithFormat:@"select * from momentsList_table_%@ where momentType = '%@' and page = '%@'", [userInfo objectForKey:@"uid"],type, page];
...
- (BOOL)deleteAllData:(NSString*)type
{
    ...
    NSString *reportSql = [NSString stringWithFormat:@"delete from momentsList_table_%@ where momentType = '%@'", [userInfo objectForKey:@"uid"],type];
    return [self executeSql:reportSql];
}
