...
@property (strong, nonatomic) IBOutlet UILabel *healthInfoLabel;
@property (strong, nonatomic) IBOutlet UILabel *userNameLabel;
@property (strong, nonatomic) IBOutlet UILabel *wishLabel;
@property (strong, nonatomic) IBOutlet UIView *imagesBaseView;
...
@property (strong, nonatomic) IBOutlet UIView *wishView;
...
- (void)viewDidLoad {
    ...
    [self setCustomizeTitle:@"身体记录详情"];
    
    [ReportObject event:ID_OPEN_BODY_DETAIL];
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(loadData)
                                                 name:@"reloadHealthDetailView"
                                               object:nil];
    ...
    if ([usertype integerValue] == 0 || [usertype integerValue] == 6) {//教师身份不可编辑
        [self setCustomizeRightButtonWithName:@"编辑"];
 ...
 * @date 2015.11.27
 * @args
 *  v=2, ac=Physical, op=condition, sid=, uid=, cid=, pid=记录ID
 */
-(void)loadData{
...
-(void)updateUI{
    ...
    NSString *dateStr = [Utilities replaceNull:[NSString stringWithFormat:@"%@",[dict objectForKey:@"dateline"]]];
   _dateLabel.text = [[Utilities alloc] linuxDateToString:dateStr andFormat:@"%@-%@-%@ %@:%@" andType:DateFormat_YMDHM];
    _heightLabel.text = [Utilities replaceNull:[NSString stringWithFormat:@"%@",[dict objectForKey:@"height"]]];
    _weightLabel.text = [Utilities replaceNull:[NSString stringWithFormat:@"%@",[dict objectForKey:@"weight"]]];
    NSDictionary *visionDic = [dict objectForKey:@"vision"];
    _leftEyeLabel.text = [Utilities replaceNull:[NSString stringWithFormat:@"%@",[visionDic objectForKey:@"left"]]];
    _rightEyeLabel.text = [Utilities replaceNull:[NSString stringWithFormat:@"%@",[visionDic objectForKey:@"right"]]];
    _healthInfoLabel.text = [Utilities replaceNull:[NSString stringWithFormat:@"%@",[dict objectForKey:@"remark"]]];
    
    NSString *userName = [Utilities replaceNull:[NSString stringWithFormat:@"%@",[dict objectForKey:@"username"]]];
    NSString *note = [Utilities replaceNull:[NSString stringWithFormat:@"%@",[dict objectForKey:@"note"]]];
    NSArray *picsArray = [dict objectForKey:@"pics"];
    
    if ([note length] > 0) {
        
        _userNameLabel.text = userName;
        _wishLabel.text = note;
        
        CGSize size = [Utilities getStringHeight:note andFont:[UIFont systemFontOfSize:15.0] andSize:CGSizeMake(winSize.width-30.0, 0)];
        _wishLabel.frame = CGRectMake(_wishLabel.frame.origin.x, _wishLabel.frame.origin.y, winSize.width-30.0, size.height);
        
        if ([picsArray count] > 0) {
            
            _imagesBaseView.frame = CGRectMake(0.0, _wishLabel.frame.origin.y+_wishLabel.frame.size.height+10, winSize.width, _imagesBaseView.frame.size.height);
            [self addMoudelView:picsArray];// 文字图片都有
        }else{
            _imagesBaseView.frame = CGRectMake(0.0, _wishLabel.frame.origin.y+_wishLabel.frame.size.height+10, winSize.width, 0.0);
            [self addMoudelView:picsArray];// 只有文字
        }
        
    }else{
        
        _userNameLabel.hidden = YES;
        _wishLabel.hidden = YES;
        
        _wishLabel.frame = CGRectMake(_wishLabel.frame.origin.x, _wishLabel.frame.origin.y, 0, 0);
        _userNameLabel.frame = CGRectMake(_userNameLabel.frame.origin.x, _userNameLabel.frame.origin.y, 0, 0);
        
        if ([picsArray count] > 0 ) {
            
          _imagesBaseView.frame = CGRectMake(0.0, 10.0, winSize.width, _imagesBaseView.frame.size.height);
          [self addMoudelView:picsArray];// 只有图片
        }else{
            _imagesBaseView.frame = CGRectMake(0.0, 10.0, winSize.width, 0.0);
            [self addMoudelView:picsArray];// 文字图片都没有 默认文言
        }
...
-(void)addMoudelView:(NSArray*)picsArray{
    ...
    NSArray *array = [_imagesBaseView subviews];
    
    for (int i = 0 ; i< [array count]; i++) {
        
        TSTouchImageView *imgV = (TSTouchImageView*)[_imagesBaseView viewWithTag:440+i];
    ...
    for (int i=0; i<[picsArray count]; i++) {
        ...
        NSString *url = [Utilities replaceNull:[NSString stringWithFormat:@"%@",[[picsArray objectAtIndex:i] objectForKey:@"url"]]];
         //NSString *picId = [Utilities replaceNull:[NSString stringWithFormat:@"%@",[[picsArray objectAtIndex:i] objectForKey:@"id"]]];
        
        TSTouchImageView *imgV =[[TSTouchImageView alloc]initWithFrame:CGRectMake(0, 0, 90.0, 90.0)];
        imgV.contentMode = UIViewContentModeScaleAspectFill;
        imgV.clipsToBounds = YES;
        imgV.userInteractionEnabled = YES;
        imgV.tag = 440+i;
        [imgV sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:@"loading_gray.png"]];
        TSTapGestureRecognizer *myTapGesture = [[TSTapGestureRecognizer alloc] initWithTarget:self action:@selector(img_btnclick:)];
        
        NSDictionary *tsDic = [NSDictionary dictionaryWithObjectsAndKeys:
                             [NSString stringWithFormat:@"%ld",(long)imgV.tag], @"tag",
                             url, @"url",
                             picsArray, @"picAry",
                             nil];
        myTapGesture.infoDic = tsDic;
        [imgV addGestureRecognizer:myTapGesture];
        ...
        imgV.frame = CGRectMake(x*90.0+15+x*10, 90.0*j+j*10, 90.0, 90.0);
        [_imagesBaseView addSubview:imgV];
        
        x++;
        
        lastY = j*90.0+j*10 + 90.0;
        
    }
    
    _imagesBaseView.frame = CGRectMake(0.0, _imagesBaseView.frame.origin.y, winSize.width, lastY);
    
    _wishView.frame = CGRectMake(0.0, _wishView.frame.origin.y, winSize.width,_imagesBaseView.frame.origin.y+ _imagesBaseView.frame.size.height+10);
    
    NSLog(@"_wishView.frame:%f",_wishView.frame.size.height);
    
#if 0 //此页加入评论页，改成tableview，原scrollview作为table的headerview
    
    if ((_wishView.frame.size.height+224.0) > (winSize.height - 64.0)) {
        
        float height = (_wishView.frame.size.height+224.0) - (winSize.height - 64.0);
    ...
    _scrollView.frame = CGRectMake(0, 0, winSize.width, _wishView.frame.size.height+224.0);
...
-(void)img_btnclick:(id)sender{
    ...
    NSDictionary *tsDic = tsTap.infoDic;
    NSArray *picAry = [tsDic objectForKey:@"picAry"];
    NSInteger tag = [[tsDic objectForKey:@"tag"] integerValue] - 440;;
    ...
    for (int i = 0; i<[picAry count]; i++) {
        // 拼字符串，去服务器获取原图
//        NSString *pic_url = [NSString stringWithFormat:REQ_PIC_URL, @"WIFI", [[[[dataArray objectAtIndex:[cellNum integerValue]] objectForKey:@"pics"] objectAtIndex:i] objectForKey:@"pid"]];
        NSString *pic_url = [Utilities replaceNull:[NSString stringWithFormat:@"%@",[[picAry objectAtIndex:i] objectForKey:@"url"]]];
        NSLog(@"picUrl:%@",pic_url);
        MJPhoto *photo = [[MJPhoto alloc] init];
        photo.save = NO;
        photo.url = [NSURL URLWithString:pic_url]; // 图片路径
        photo.srcImageView = [_imagesBaseView viewWithTag:tag+440]; // 来源于哪个UIImageView
