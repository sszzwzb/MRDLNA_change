...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    NSString *name = [personalInfo objectForKey:@"reason"];
...
- (void)loadView
{
    ...
    {
        text_title = [[UITextField alloc]initWithFrame:CGRectMake(15, 24, WIDTH-30, 50)];
    }

    text_title.borderStyle = UITextBorderStyleNone;
    text_title.backgroundColor = [UIColor clearColor];
    text_title.placeholder = @"请输入申请说明";
...
{
    if (text_title.text.length >= 40) {
        UIAlertView* alert = [[UIAlertView alloc]initWithTitle:@"错误"
                                                       message:@"申请理由超过40位，请重新输入。"
                                                      delegate:nil
        ...
        NSMutableDictionary *personalInfo = [g_userInfo getUserPersonalInfo];
        [personalInfo setObject:text_title.text forKey:@"reason"];
