...
-(void)viewWillAppear:(BOOL)animated{
    ...
    // 查看全部活动列表
    [ReportObject event:ID_OPEN_ALL_EVENT];// 2015.06.13
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //NSString *joined = [eidList objectAtIndex:indexPath.section];
...
//        eventType = [message_info objectForKey:@"type"];
        ...
        //刷新表格内容
        if ([eventArray count] >0) {//lastId_cnews
            
           // NSString *eid = [NSString stringWithFormat:@"%@",[eidList objectAtIndex:0]];
           // [[NSUserDefaults standardUserDefaults]setObject:eid forKey:@"lastId_cnews"];
            NSString *newsId = [NSString stringWithFormat:@"%@",[eidList objectAtIndex:0]];
