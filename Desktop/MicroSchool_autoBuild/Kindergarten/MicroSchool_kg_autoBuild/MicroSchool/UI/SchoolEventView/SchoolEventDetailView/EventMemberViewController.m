...
@interface EventMemberViewController ()

@end

@implementation EventMemberViewController
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    // 查看活动成员列表
    [ReportObject event:ID_OPEN_EVENT_MEMBER];//2015.06.23
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    EventMemberCell *cell = [tableView dequeueReusableCellWithIdentifier:CellTableIdentifier];
    if(cell == nil) {
        cell = [[EventMemberCell alloc]
    ...
    NSString* sex= [list_dic objectForKey:@"sex"];
    NSString* note= [list_dic objectForKey:@"note"];
    
    //Utilities *util = [Utilities alloc];
    
    //-----update by kate 2014.11.14---------------------------------------------------------------
    NSString *head_url = [list_dic objectForKey:@"avatar"];
    //NSString* head_url = [util getAvatarFromUid:uid1 andType:@"1"];
    //---------------------------------------------------------------------------
...
-(void)createHeaderView
{
     ...
     *  2015.04.21
     * @args
     *  op=members, sid=, uid=, eid=, page=, size=
     */
