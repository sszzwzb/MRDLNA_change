...
@interface SubscribePeopleViewController ()
...
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(doCancelFollowing:) name:@"Weixiao_knowledgeCancelFollowing" object:nil];
}

-(void)viewWillDisappear:(BOOL)animated
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"Weixiao_knowledgeCancelFollowing" object:nil];
}

-(void)doCancelFollowing:(NSNotification *)notification
{
    NSLog(@"doCancelFollowing");
    ...
    [network sendHttpReq:HttpReq_WikiFollowCancel andData:data];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SubscribePeopleTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellTableIdentifier];
    if(cell == nil) {
        cell = [[SubscribePeopleTableViewCell alloc]
...
//    NSDictionary *dic = [NSDictionary dictionaryWithObject:@"subPeopleView" forKey:@"tid"];
//    
...
//                          self->endNum, @"size",
    ...
    [network sendHttpReq:HttpReq_WikiFollowing andData:data];
}
...
- (void)reciveHttpData:(NSData*)data andType:(HttpReqType)type
{
    ...
    if([@"do_wiki_following" isEqualToString:[resultJSON objectForKey:@"protocol"]])
    {
        if(true == [result intValue])
        {
            ...
            for (NSObject *object in temp)
            {
                ...
                [eidList addObject:[aaa objectForKey:@"uid"]];
            ...
            if ([listDataArray count] > 0) {
                ...
                noDataView = [Utilities showNodataView:@"暂未订阅名师" msg2:@"订阅作者跟踪最新知识库" andRect:rect];
    ...
    else if ([@"do_wiki_followingCancel" isEqualToString:[resultJSON objectForKey:@"protocol"]])
    {
