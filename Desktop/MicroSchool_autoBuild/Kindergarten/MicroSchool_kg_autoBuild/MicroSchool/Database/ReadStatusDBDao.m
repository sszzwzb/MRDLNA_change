...
//业务库实例
static ReadStatusDBDao *gDaoInstance = nil;

@implementation ReadStatusDBDao
#pragma mark -
#pragma mark 属性
...
#pragma mark -
#pragma mark 数据库实例
+ (ReadStatusDBDao *)getDaoInstance
{
    @synchronized(self)
    {
        if (gDaoInstance == nil) {
            gDaoInstance = [[ReadStatusDBDao alloc] init];
            //初始化数据库名字
            [gDaoInstance genDBPath:[NSString stringWithFormat:@"ReadStatus.db"]];
        }
        //创建所有基本表
...
- (BOOL)deleteDBFile
{
    NSString *dbFilePath = [[Utilities SystemDir] stringByAppendingPathComponent:@"ReadStatus.db"];
...
{
    //创建上报表
    NSString *reportSql = [DBTableCreate getReadStatusSql];
...
- (NSMutableArray *)getAllData
{
    ...
    NSString *sql = [NSString stringWithFormat:@"select * from readStatus_table_%@", [userInfo objectForKey:@"uid"]];
...
- (NSMutableDictionary *)getDataFromReadId:(NSString *)readId
{
    NSDictionary *userInfo = [[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"weixiao_userDetailInfo"]];
    
    NSString *sql = [NSString stringWithFormat:@"select * from readStatus_table_%@ where readId = '%@'", [userInfo objectForKey:@"uid"], readId];
    NSMutableDictionary *dict = [self getDictionaryResultsByColumnName:sql];
...
- (BOOL)deleteAllData
{
    ...
    NSString *reportSql = [NSString stringWithFormat:@"delete from readStatus_table_%@", [userInfo objectForKey:@"uid"]];
    return [self executeSql:reportSql];
}
