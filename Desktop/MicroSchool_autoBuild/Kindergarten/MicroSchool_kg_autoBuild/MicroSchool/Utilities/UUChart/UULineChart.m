...
- (void)addPoint:(CGPoint)point index:(NSInteger)index isShow:(BOOL)isHollow value:(CGFloat)value
{
    ...
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake(5, 7, 6, 6)];
    view.center = point;
...
//    }
    ...
    //view.backgroundColor = [_colors objectAtIndex:index]?[_colors objectAtIndex:index]:UUGreen;
    view.backgroundColor = [_colors objectAtIndex:index]?[_colors objectAtIndex:index]:UUWhite;
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point.x-UUTagLabelwidth/2.0, point.y-UULabelHeight*2+6, UUTagLabelwidth, UULabelHeight)];
    if ((int)value < 3 || point.y < 13.5) {//图标第一行的点显示不全修正
    
        label.frame = CGRectMake(label.frame.origin.x, point.y-UULabelHeight*2+8, UUTagLabelwidth, UULabelHeight);
    }
    ...
    NSLog(@"point%f-value%f:",point.y,value);
