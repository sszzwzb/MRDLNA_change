...
@interface KYLandScapeControlView () <KYSliderViewDelegate>
/// 顶部工具栏
...
/// 滑杆
@property (nonatomic, strong) KYSliderView *slider;
/// 视频总时间
...
@implementation KYLandScapeControlView
...
#pragma mark - KYSliderViewDelegate
...
- (void)sliderTouchEnded:(float)value {
    if (self.player.totalTime > 0) {
        ...
        self.slider.isdragging = NO;
        self.slider.value = 0;
        self.slider.curLastVideoTimeValue = -1;
    }
}
...
- (void)videoPlayer:(ZFPlayerController *)videoPlayer currentTime:(NSTimeInterval)currentTime totalTime:(NSTimeInterval)totalTime {
    if (!self.slider.isdragging) {
        ...
        self.slider.curLastVideoTimeValue = _curLastVideoTimeValue;
    }
}
...
- (KYSliderView *)slider {
    if (!_slider) {
        _slider = [[KYSliderView alloc] init];
