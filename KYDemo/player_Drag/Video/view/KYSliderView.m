...
/** 滑块 */
@property (nonatomic, strong) KYSliderButton *sliderBtn;
...
- (void)sliderBtnDragMoving:(UIButton *)btn point:(CGPoint)touchPoint {
    ...
    // 最长播放时间
    if (value >= self.curLastVideoTimeValue && self.curLastVideoTimeValue > 0) {
        value = self.curLastVideoTimeValue;
        if ([self.delegate respondsToSelector:@selector(doNotSlideToUnseen)]) {
            [self.delegate doNotSlideToUnseen];
        }
    }
...
- (KYSliderButton *)sliderBtn {
    if (!_sliderBtn) {
        _sliderBtn = [KYSliderButton buttonWithType:UIButtonTypeCustom];
