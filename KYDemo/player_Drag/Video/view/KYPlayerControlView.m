...
#import "KYPortraitControlView.h"
#import "KYLandScapeControlView.h"
#import "KYSliderView.h"


@interface KYPlayerControlView () <KYSliderViewDelegate>
/// 竖屏控制层的View
@property (nonatomic, strong) KYPortraitControlView *portraitControlView;
/// 横屏控制层的View
@property (nonatomic, strong) KYLandScapeControlView *landScapeControlView;
/// 加载loading
@property (nonatomic, strong) ZFSpeedLoadingView *activity;
/// 快进快退View
@property (nonatomic, strong) UIView *fastView;
/// 快进快退进度progress
@property (nonatomic, strong) KYSliderView *fastProgressView;
/// 快进快退时间
...
/// 底部播放进度
@property (nonatomic, strong) KYSliderView *bottomPgrogress;
/// 是否显示了控制层
...
@implementation KYPlayerControlView
...
- (void)gestureChangedPan:(ZFPlayerGestureControl *)gestureControl panDirection:(ZFPanDirection)direction panLocation:(ZFPanLocation)location withVelocity:(CGPoint)velocity {
    if (direction == ZFPanDirectionH) {
        ...
        // 最长播放时间
        if (self.sumTime >= self.curLastVideoTime && self.curLastVideoTime > 0) {
            self.sumTime = self.curLastVideoTime;
            [self doNotSlideToUnseen];
        }
        // 需要限定sumTime的范围
...
- (void)videoPlayer:(ZFPlayerController *)videoPlayer currentTime:(NSTimeInterval)currentTime totalTime:(NSTimeInterval)totalTime {
    ...
    //随着时间边长，可以后滑动的时间变长
    if (self.curLastVideoTime < currentTime) self.curLastVideoTime = currentTime;
    self.portraitControlView.curLastVideoTimeValue = self.curLastVideoTime / totalTime;
    self.landScapeControlView.curLastVideoTimeValue = self.curLastVideoTime / totalTime;
...
- (KYPortraitControlView *)portraitControlView {
    if (!_portraitControlView) {
        @zf_weakify(self)
        _portraitControlView = [[KYPortraitControlView alloc] init];
...
- (KYLandScapeControlView *)landScapeControlView {
    if (!_landScapeControlView) {
        @zf_weakify(self)
        _landScapeControlView = [[KYLandScapeControlView alloc] init];
...
- (KYSliderView *)fastProgressView {
    if (!_fastProgressView) {
        _fastProgressView = [[KYSliderView alloc] init];
...
- (KYSliderView *)bottomPgrogress {
    if (!_bottomPgrogress) {
        _bottomPgrogress = [[KYSliderView alloc] init];
